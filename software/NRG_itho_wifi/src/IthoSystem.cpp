...
// FF checksum
// Leave timestamp and temp1 zero to just set temp2.

{
  ...
  jsonSysmessage("ithoCE30sendcommand", i2cbuf2string(command, sizeof(command)).c_str());

  if (!i2c_sendBytes(command, sizeof(command), I2C_CMD_SEND_CE30))
  {
    if (updateweb)
    {
      updateweb = false;
      jsonSysmessage("ithoCE30result", "failed");
    }
    return;
  }
  if (updateweb)
  {
    updateweb = false;
    jsonSysmessage("ithoCE30result", "CE30 Broadcast sent.");
  }
}
